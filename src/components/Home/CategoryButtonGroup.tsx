...
const categoryData = ['For You', 'Reality', 'Big Boss', 'Top Free Movies', 'Premium', 'Top 10', 'Studios', 'Live', 'Studios'];
        ...
        <Container>
            <div className='flex justify-between overflow-hidden gap-x-[18px] my-[43px]'>
                {categoryData.map((category, index) => (
                    <button
                        key={index}
                        className={`px-6 py-3 rounded-full font-bold text-[16px] ${category === 'For You'
                ...
                >
                    <div className='bg-[#F3CF9F] w-[27px] h-[27px] flex justify-center items-center rounded-full'>
                        <img src="/public/assets/arrowright.svg" alt="" />
                    </div>
                </button>
            </div>
        </Container>
